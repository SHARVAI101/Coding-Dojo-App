...
import 'package:coding_dojo_app/beginner_theory/nmb_q_1_theory.dart';
...
class NumbersQuestion1 extends StatefulWidget {
  @override
  _NumbersQuestion1State createState() => _NumbersQuestion1State();
}

class _NumbersQuestion1State extends State<NumbersQuestion1> {
  ...
  var options = [["num/2 == 0", 0],["num/3 == 1",0],["num%2 == 0",0],["num^3/2==1",0]]; // 0: Answer text, 1: 1=selected 0=not selected
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            RichText(
              text: TextSpan(
                  text: "Given a variable ",
                  style: TextStyle( fontFamily: 'FreeSans', fontSize: 15, color:Colors.black),
                  children: [
                    TextSpan(text: 'num', style: TextStyle(color:Colors.pink)),
                    TextSpan(text: ', find if it is even or odd and print the result.'),
                  ]
              ),
            ),
            SizedBox(height: 10),
                ...
                SizedBox(
                  ...
                  child: RaisedButton(
                    onPressed: (){
                      Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => NumbersQuestion1Theory()),
                      );
                    },
                  ...
                  child: RaisedButton(
                    onPressed: (allBlanksFilled==false)?null:() async {
                      ...
                      if(_isCorrect==true){
                        ...
                        SharedPreferences prefs = await SharedPreferences.getInstance();
                        var completed_nmb_questions=[];
                        if(prefs.getString("completed_nmb_questions")!=null) {
                          print("notnull");
                          completed_nmb_questions = jsonDecode(prefs.getString("completed_nmb_questions"));
                        }
                        int index = completed_nmb_questions.indexOf(uniqueID);
                        print(index);
                        if(index<0) {
                          completed_nmb_questions.add(uniqueID);
                          _isAlreadySubmitted=false;
                        }
                        prefs.setString("completed_nmb_questions", jsonEncode(completed_nmb_questions));
                        print(jsonDecode(prefs.getString("completed_nmb_questions")));
