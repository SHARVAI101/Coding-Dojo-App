import 'dart:convert';
import 'package:coding_dojo_app/beginner_theory/basic_q_1_point_5_theory.dart';
import 'package:coding_dojo_app/feedback1.dart';
import 'package:coding_dojo_app/feedback2.dart';
...
class BasicQuestion1point5 extends StatefulWidget {
  @override
  _BasicQuestion1point5State createState() => _BasicQuestion1point5State();
}

class _BasicQuestion1point5State extends State<BasicQuestion1point5> {

  var blanks = [[1, 0, -1],[0, 0, -1]];
  // 0: Correct answer index from options,
  // 1: 0=not filled 1=filled,
  // 2: index of what is currently filled in this blank

  var options = [["b", 0],["sum",0]]; // 0: Answer text, 1: 1=selected 0=not selected
  ...
  Future<void> checkFeedbackSeen() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    bool _onSeen = (prefs.getBool('feedbackSeen') ?? false);
    // prefs.remove("feedbackSeen");
    print(_onSeen);
    if (_onSeen==false) {
      prefs.setBool('feedbackSeen', true);
      Navigator.push(
        context,
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            RichText(
              text: TextSpan(
                  text: "You have 3 integer variables ",
                  ...
                  children: [
                    ...
                    TextSpan(text: ' and '),
                    TextSpan(text: 'sum', style: TextStyle(color:Colors.pink)),
                    TextSpan(text: '.'),
                  ]
              ),
            ),
            RichText(
              text: TextSpan(
                  ...
                  children: [
                    ...
                    TextSpan(text: ' and '),
                    TextSpan(text: 'b', style: TextStyle(color:Colors.pink)),
                    TextSpan(text: ' and store the result in '),
                    TextSpan(text: 'sum', style: TextStyle(color:Colors.pink)),
                    TextSpan(text: '.'),
                  ]
              ),
                ...
                SizedBox(
                  ...
                  child: RaisedButton(
                    onPressed: (){
                      Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => BasicQuestion1point5Theory()),
                      );
                    },
